...
from diffbir.model.lora import LoRALinear
...
class ControlLDM(nn.Module):
    ...
    def inject_lora(self, rank_unet=64, rank_controlnet=16):
        def replace_linear(module, rank):
            for name, child in module.named_children():
                if isinstance(child, nn.Linear):
                    setattr(module, name, LoRALinear(child, rank=rank))
                else:
                    replace_linear(child, rank)

        print("Injecting LoRA into UNet...")
        replace_linear(self.unet, rank_unet)
