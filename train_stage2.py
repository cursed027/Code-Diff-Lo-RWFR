    ...
    while global_step < max_steps:
        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                ...
                # Curriculum noise augmentation (SAFE)
                if global_step < 3000:
                    noise_t = 30
                ...
                if noise_t > 0:
                    cond_aug["c_img"] = diffusion.q_sample(
                        cond_aug["c_img"],
                        torch.randint(0, noise_t, (z0.size(0),), device=device),
            ...
            global_step += 1
            step_diff.append(loss.item())
    
            pbar.set_description(
                f"step={global_step} diff={loss.item():.3f}"
            )
    
            if writer and global_step % cfg.train.log_every == 0:
                writer.add_scalar(
                    "loss/diffusion",
                    sum(step_diff) / len(step_diff),
                    global_step
                )
                step_diff.clear()
